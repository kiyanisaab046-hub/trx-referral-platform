...
export default function CommunityInfoPage() {
  ...
  const [levelData, setLevelData] = useState<Record<number, CommunityMember[]>>({});
  ...
  useEffect(() => {
    const fetchTree = async () => {
      try {
        ...
        // We separate into directs and indirects just like My Team page
        ...
        // Root represents the authUser (L0)
        let poolIndex = 0;
        let currentLevelQueue = [authUser.id];
        let level = 1;
        const resultLevels: Record<number, CommunityMember[]> = {};
      ...
      <header className={styles.header}>
        <div className={styles.logoArea} onClick={() => router.push('/dashboard')} style={{ cursor: 'pointer' }}>
          ...
          <div className={styles.logoTitles}>
            ...
            <span className={styles.logoSlogan}>Binary Tree Levels</span>
          </div>
        </div>
      ...
      <main className={styles.mainContent} style={{ flex: 1, padding: '2rem 1rem' }}>
        {loading ? (
          ...
          <div style={{ background: 'rgba(10,15,30,0.6)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)', overflow: 'hidden' }}>
            ...
            {/* Horizontal Tabs */}
            <div style={{ display: 'flex', overflowX: 'auto', background: '#1c2237', borderBottom: '2px solid #000' }}>
                ...
                <tbody>
                  {(levelData[activeTab] || []).length > 0 ? (
                    levelData[activeTab].map((member, index) => (
                      <tr key={member.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', color: '#e2e8f0' }}>
            ...
            <div style={{ padding: '1rem', background: 'rgba(0,0,0,0.2)', textAlign: 'right', fontSize: '0.8rem', color: '#8892b0' }}>
